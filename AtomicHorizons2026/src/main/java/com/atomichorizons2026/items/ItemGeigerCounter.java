...
public class ItemGeigerCounter extends ItemBase {
    ...
    public ActionResult<ItemStack> onItemRightClick(World world, EntityPlayer player, EnumHand hand) {
        ...
        player.sendMessage(new TextComponentString(
            TextFormatting.GREEN + "=== Geiger Counter ==="));
        player.sendMessage(new TextComponentString(
            TextFormatting.GRAY + "Chunk Radiation: " + 
            color + radiationText));
    ...
    private String getRadiationStatus(double radiation) {
        if (radiation < 1.0) return TextFormatting.GREEN + "SAFE";
        if (radiation < 10.0) return TextFormatting.YELLOW + "ELEVATED";
        if (radiation < 100.0) return TextFormatting.GOLD + "DANGEROUS";
        if (radiation < 500.0) return TextFormatting.RED + "CRITICAL";
        return TextFormatting.DARK_RED + "LETHAL";
    }
}
