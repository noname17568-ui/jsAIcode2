...
public class ItemRadAway extends ItemFood {
    ...
    public ItemRadAway(RadAwayType type) {
        ...
        setTranslationKey(type.getName());
        ...
        if (cap == null || cap.getRadiationDose() <= 0) {
            if (!world.isRemote) {
                player.sendMessage(new TextComponentString(
                    TextFormatting.GREEN + "No radiation detected"));
            }
    ...
    public ItemStack onItemUseFinish(ItemStack stack, World world, EntityPlayer player) {
        ...
        if (cap != null) {
            ...
            player.sendMessage(new TextComponentString(
                ...
                "Radiation reduced by " + String.format("%.1f", reduced) + " RAD"));
            
            // Оставшаяся доза
            if (newDose > 0) {
                player.sendMessage(new TextComponentString(
                    TextFormatting.YELLOW + 
                    "Remaining dose: " + String.format("%.1f", newDose) + " RAD"));
            } else {
                player.sendMessage(new TextComponentString(
                    TextFormatting.GREEN + 
                    "Radiation fully cleared!"));
            }
        }
