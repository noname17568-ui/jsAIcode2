...
/**
 * Industrial Smelter - Tier 1 ore processing.
 * Smelts enriched dust into ingots (1 dust -> 1 ingot)
 * Per TZ Section 2.2
 */
public class TileEntityIndustrialSmelter extends TileEntity implements ITickable {

    // Energy
    private EnergyStorage energyStorage = new EnergyStorage(150_000, 1_500, 0);
    ...
    private boolean canProcess(ItemStack input) {
        return !com.atomichorizons2026.crafting.MachineRecipeRegistry.getSmelterOutput(input).isEmpty();
    }

    private boolean canFitOutput(ItemStack input) {
        ItemStack output = com.atomichorizons2026.crafting.MachineRecipeRegistry.getSmelterOutput(input);
