...
/**
 * Step-Down Transformer TileEntity.
 * Входит: 1024 RF/t, Выходит: 256 RF/t (4:1 ratio)
 */
public class TileEntityStepDownTransformer extends TileEntity implements ITickable {
    
    private static final int INPUT_CAPACITY = 10000; // Входной буфер
    private static final int OUTPUT_CAPACITY = 2500; // Выходной буфер
    private static final int CONVERSION_RATIO = 4; // 4:1 (1024 → 256)
    
    private EnergyStorage inputStorage = new EnergyStorage(INPUT_CAPACITY, 1024, 0, 0);
    private EnergyStorage outputStorage = new EnergyStorage(OUTPUT_CAPACITY, 0, 256, 0);
    ...
    public void update() {
        ...
        if (inputEnergy > 0 && outputSpace > 0) {
            int toConvert = Math.min(inputEnergy, outputSpace * CONVERSION_RATIO) / CONVERSION_RATIO;
            
            inputStorage.extractEnergy(toConvert * CONVERSION_RATIO, false);
            outputStorage.receiveEnergy(toConvert, false);
        ...
        for (EnumFacing facing : EnumFacing.VALUES) {
            ...
            if (neighbor != null && neighbor.hasCapability(CapabilityEnergy.ENERGY, facing.getOpposite())) {
                int sent = neighbor.getCapability(CapabilityEnergy.ENERGY, facing.getOpposite())
                    .receiveEnergy(outputStorage.extractEnergy(256, false), false);
