...
/**
 * Distillery TileEntity.
 * Очищает и разделяет жидкости.
 * Потребление: 80 RF/t
 */
public class TileEntityDistillery extends TileEntity implements ITickable {
    
    private static final int FLUID_CAPACITY = 10000; // 10000 mB
    private static final int MAX_ENERGY = 100_000; // 100k RF
    private static final int ENERGY_PER_TICK = 80; // 80 RF/t
    ...
    public void update() {
        ...
        if (energyStorage.getEnergyStored() >= ENERGY_PER_TICK && 
            ...
            // Очищаем жидкость: берём тип из inputTank и переносим в output
            FluidStack drained = inputTank.drain(15, false);
            if (drained != null) {
                int filled = outputTank.fill(drained, false);
                if (filled > 0) {
                    inputTank.drain(filled, true);
                    outputTank.fill(drained, true);
